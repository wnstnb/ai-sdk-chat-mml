...
✅ 4.  **Extract `AutosaveStatusIndicator` Component:** Create `components/editor/AutosaveStatusIndicator.tsx`. Pass `autosaveStatus` as a prop. Replace the inline JSX in `page.tsx`.
❌ 5.  **Extract `useAutosave` Hook:** Move autosave state (`autosaveTimerId`, `revertStatusTimerId`, `autosaveStatus`, `latestEditorContentRef`, `latestEditorBlocksRef`, `isContentLoadedRef`), the `triggerSaveDocument` logic (API call), and the `handleEditorChange` logic into `lib/hooks/editor/useAutosave.ts`. Update `page.tsx` to use the hook, passing `editorRef` and `documentId`. Pass the returned `handleEditorChange` to `BlockNoteEditorComponent`.
❌ 6.  **Extract `useNavigationSave` Hook:** Move the `beforeunload` and navigation `useEffect` hooks related to saving into `lib/hooks/editor/useNavigationSave.ts`. It needs `documentId`, `autosaveStatus` (from `useAutosave`), and `latestEditorContentRef`. Update `page.tsx` to call this hook.
...
8.  **Extract `EditorTitleBar` Component:** Create `components/editor/EditorTitleBar.tsx`. Move the title bar JSX. Pass props from `useTitleManagement` (state and handlers), `autosaveStatus` (from `useAutosave`), manual save handler (defined in `page.tsx` or moved), `isSaving` state, new doc handler, etc. Use the `AutosaveStatusIndicator` component inside.
...
16. **Extract `EditorPaneWrapper` Component:** Create `components/editor/EditorPaneWrapper.tsx`. Move the editor container div and the `BlockNoteEditorComponent`. Needs initial content (from `useDocument`), `editorRef`, `onEditorContentChange` (from `useAutosave`). May also include `CollapsedChatInput`.
17. **Refine `page.tsx`:** The main component should now primarily compose the extracted hooks and components, passing props between them. State like `model` might live here, or within `useChatInteractions`. It orchestrates the hooks and renders the main layout (`EditorPaneWrapper`, `ChatPaneWrapper`).
...
*   **API Interaction:** This refactoring focuses on client-side structure. **It should not change the signatures of API calls** (endpoints, request methods, body/response structure). API calls will simply be located within the relevant hook (e.g., save in `useAutosave`, fetch doc in `useDocument`).

...
export function useFolders(): UseFoldersReturn {
  ...
  const fetchFolders = useCallback(async () => {
    ...
    setError(null);
    try {
      // Fetch flat list of folders
      const response = await fetch('/api/folders');
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(errorData.error?.message || `Failed to fetch folders (${response.status})`);
      }
      const { data } = await response.json();
      const flatFolders = data?.folders || [];
      setFolders(flatFolders);

      // Fetch hierarchical structure
      const hierarchicalResponse = await fetch('/api/folders?hierarchical=true');
      if (!hierarchicalResponse.ok) {
        const errorData = await hierarchicalResponse.json().catch(() => ({}));
        throw new Error(errorData.error?.message || `Failed to fetch folder hierarchy (${hierarchicalResponse.status})`);
      }
      const { data: hierarchicalData } = await hierarchicalResponse.json();
      setFolderTree(hierarchicalData?.folders || []);
    } catch (err: any) {

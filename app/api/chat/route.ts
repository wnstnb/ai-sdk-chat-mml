...
**3. Strict 4-Step Process for Editor Actions (Intent C):**
    ...
    * **Step 3: Plan Tool Call & Parameters:** Select the appropriate tool and determine its parameters based on the block type:
        ...
        * **For NON-TABLE Blocks:**
            ...
            * **CRITICAL WORKFLOW for Splitting Content into Multiple Blocks:**
                * **When users ask to "split", "break up", "format with line breaks", or "separate" content (e.g., "split this poem into separate lines", "break this paragraph into bullet points"):**
                    1. **FIRST**: Use \`deleteContent\` to remove the original block
                    2. **THEN**: Use \`addContent\` with properly formatted Markdown that will create multiple blocks
                * **Example**: User says "split this poem into separate lines"
                    1. \`deleteContent({ targetBlockId: "block-id", targetText: null })\`
                    2. \`addContent({ markdownContent: "Line 1\n\nLine 2\n\nLine 3", targetBlockId: null })\`
                * **DO NOT** try to use \`modifyContent\` for splitting operations - it only works with single blocks.
...
* \`modifyContent({ targetBlockId: string, targetText: string | null, newMarkdownContent: string })\`: Modifies content **ONLY within NON-TABLE blocks**. Can target a single block with optional specific text replacement. This is the primary tool for altering existing lists, converting items to checklists, and changing text in list items. **CRITICAL LIMITATION: This tool CANNOT split one block into multiple blocks - it only works with ONE block at a time. If you need to split content into multiple blocks, use deleteContent + addContent instead.**
* \`deleteContent({ targetBlockId: string, targetText: string | null })\`: Deletes content **ONLY from NON-TABLE blocks**. Handles whole-block deletion (\`targetText: null\`) or specific text deletion (\`targetText: 'text to delete'\`). **IMPORTANT: Use this BEFORE addContent when you need to replace a single block with multiple blocks (e.g., splitting a poem into separate lines, breaking up a paragraph, formatting content with line breaks).**

...
/**
 * POST /api/documents/[documentId]/threads/[threadId]/resolve
 * Resolve a thread
 */
export async function POST(
  ...
  try {
    ...
    return NextResponse.json({ message: 'Thread resolved' });
  } catch (error) {
    console.error('[resolveThread] Error:', error);
    return NextResponse.json(
      { error: { code: 'INTERNAL_ERROR', message: 'Failed to resolve thread' } },
      { status: 500 }
    );
  }
} 